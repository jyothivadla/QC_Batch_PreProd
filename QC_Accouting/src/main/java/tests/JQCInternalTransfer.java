...
public class JQCInternalTransfer extends QCStore {
	...
	{
		try{
			int lastrow=TestData.getLastRow((Vprop.getProperty("Deposit")));
			String sheetName=Vprop.getProperty("Deposit");
			...
			{		
				...
				{		       
					...
					driver.findElement(locator(Vprop.getProperty("cash_management"))).click();
					driver.switchTo().defaultContent();
					driver.switchTo().frame("mainFrame");
					Actions action = new Actions(driver);  
					WebElement safe=driver.findElement(locator(Vprop.getProperty("safe")));
					action.moveToElement(safe).build().perform();
					test.log(LogStatus.PASS, " Click on safe menu");
					Thread.sleep(2000);
					driver.findElement(locator(Vprop.getProperty("Internal_Transfer"))).click();
					test.log(LogStatus.PASS, " click on Internal_Transfer menu");
					Thread.sleep(2000);
					driver.switchTo().defaultContent();
					driver.switchTo().frame("mainFrame");
					driver.switchTo().frame("main");
					driver.findElement(locator(Vprop.getProperty("Internal_Transfet_Action"))).sendKeys(Action);
					driver.findElement(locator(Vprop.getProperty("Count_Coins"))).sendKeys(CountofDollarCoins);
					String check_Nbr=driver.findElement(locator(Vprop.getProperty("Internal_check_Nbr"))).getText();
					...
					{
						driver.findElement(locator(Vprop.getProperty("Internal_select"))).click();
						driver.findElement(locator(Vprop.getProperty("Internal_Banker_Pin"))).sendKeys(BankerPIN);
						driver.findElement(locator(Vprop.getProperty("Internal_transfer"))).click();
						test.log(LogStatus.PASS, " Internal_transfer from CSR is successfull");
						if(driver.findElement(locator(Vprop.getProperty("Internal_ok"))).isDisplayed())
						{
